...
const toggleLGA = (element) => {
    ...
    let stateValue = element.value;
    let lgaSelect = element.parentElement.parentElement.parentElement.parentElement.querySelector(".select-lga");
    let lgaOptionsArray = ["Select LGA...", ...Object.values(lgaOptions[stateValue])];
    let lgaSelectOptionsCount = lgaSelect.options.length;

    for (let i = lgaSelectOptionsCount - 1; i >= 0; i--) {
        lgaSelect.options[i] = null;
    }

    lgaOptionsArray.forEach(optionValue => {
        let option = document.createElement("option");
        option.appendChild(document.createTextNode(optionValue));
        option.value = optionValue;

        if (optionValue.includes("Select")) {
            setAttributes(option, { disabled: "disabled", selected: "selected" });
        }
